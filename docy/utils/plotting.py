...
def plot_map(maps, locs, ax=None, cmap_order=None):
    ...
    if ax is None:
        ...
        ax.cla()
    tmp = np.zeros((3, world_size[0], world_size[1]))  # 도화지
    cmap = plt.get_cmap("jet", n_agents)  # 색깔 (n_agents)개 가져오기
    ...
    for entity in scenario_entities:
        if entity == "House":
            ...
            if resources.get(entity).collectible:
                ...
                """
                entity_color = resources.get(entity).color[:, None, None]
                entity_map = np.array(maps.get(entity))[None]
                map_ = entity_color * entity_map
                map_ /= max_health[entity]  # health에 다라 색깔 조절(but 어차피 health==1)
                tmp += map_  # 도화지에 색칠
    ...
    for i in range(n_agents):
        ...
        agent += houses
        """
        agent == house: agent 뭐하러 만든지 모르겠음
        house               | (25, 25)
        agent               | (25, 25)
    ...
    tmp = np.transpose(tmp, [1, 2, 0])  # RGB -> GBR
    tmp = np.minimum(tmp, 1.0)  # 0~255 -> 0~1
...
def do_plot(env, ax, fig, t, show=True, save=False):
